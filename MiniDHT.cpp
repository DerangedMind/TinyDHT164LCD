 ...
 *          number of sensors
 */
MiniDHT::MiniDHT(uint8_t pin, uint8_t type) {
 ...
 */
void MiniDHT::begin(uint8_t usec) {
  // set up the pins!
 ...
 */
float MiniDHT::readTemperature(bool S, bool force) {
  float f;
...
//  */
// float MiniDHT::convertCtoF(float c) { return c * 1.8 + 32; }
 ...
 */
float MiniDHT::readHumidity(bool force) {
  float f;
  if (read(force)) {
    f = data[0] + data[1] * 0.1;
  }
...
uint32_t MiniDHT::expectPulse(bool level) {
